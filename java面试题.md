...
- void add(int index, Object o)  存：
  ...
  ```
  	与一般的数组操作元素一样，先判断数组的大小是否足够，然后插入到响应位置。若插入到数组末尾，只需要将元素放入后size++；如若要插入到指定位置，需要将其后面的元素全部往后移一位在进行插入。
  ```
...
```java
 ...
 static final int hash(Object key) {
    	...
    	创建hashmap时，数组的默认长度是16。
  		1、首先hashmap方法接收到key和value时会根据put的key值进行hash运算的到key对应的hash值，是为了让key的hash值与其无符号右移					16位后的值进行运算（即将该hashcode的高位和地位全部参与到运算中，使计算出来的哈希值更加散列）。
...
|                   五种常用的存储数据结构图                   |
| :----------------------------------------------------------: |
| ![](/Users/why/Desktop/github仓库/Pictures/1586759101828.png) |

#### 	(2)缓存穿透、缓存击穿、缓存雪崩的概念以及对应的解决方案

| 缓存穿透                                                     |
| :----------------------------------------------------------- |
| ![](/Users/why/Desktop/github仓库/Pictures/1586949401099.png) |
